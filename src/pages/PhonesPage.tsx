...
const PhonesPage = () => {
  ...
  const brands = useMemo(() => [...new Set(phones.map(p => p.brand))], [phones]);
      ...
      <div className="container py-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            {isLoading ? 'Loading...' : `${filteredPhones.length} phones available for home experience`}
          </p>
        </div>
        ...
        <div className="flex gap-8">
          ...
          <aside className="hidden lg:block w-64 shrink-0">
            <div className="sticky top-24 bg-card rounded-2xl border border-border/50 p-6">
              <PhoneFilters filters={filters} onChange={setFilters} onClear={clearFilters} brands={brands} />
            </div>
          </aside>
            ...
            {(filters.brands.length > 0 || filters.os.length > 0 || filters.ram.length > 0 || filters.storage.length > 0) && (
              <div className="flex flex-wrap gap-2 mb-6">
                {filters.brands.map(brand => (
                  <Button key={brand} variant="secondary" size="sm" onClick={() => setFilters(f => ({ ...f, brands: f.brands.filter(b => b !== brand) }))}>
                    {brand}<X className="h-3 w-3 ml-1" />
                  </Button>
                ))}
                {filters.os.map(os => (
                  <Button key={os} variant="secondary" size="sm" onClick={() => setFilters(f => ({ ...f, os: f.os.filter(o => o !== os) }))}>
                    {os}<X className="h-3 w-3 ml-1" />
                  </Button>
                ))}
                {filters.ram.map(ram => (
                  <Button key={ram} variant="secondary" size="sm" onClick={() => setFilters(f => ({ ...f, ram: f.ram.filter(r => r !== ram) }))}>
                    {ram}<X className="h-3 w-3 ml-1" />
                  </Button>
                ))}
                {filters.storage.map(storage => (
                  <Button key={storage} variant="secondary" size="sm" onClick={() => setFilters(f => ({ ...f, storage: f.storage.filter(s => s !== storage) }))}>
                    {storage}<X className="h-3 w-3 ml-1" />
                  </Button>
                ))}
              </div>
            )}
